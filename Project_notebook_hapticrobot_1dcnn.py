    ...
    for p_id in range(1, 27): 
        ...
        for hfile_path in haptic_files_glob:
            ...
            try:
                ...
                max_len = max(len(haptic_data), len(robot_data))
                haptic_data = pad_with_last_value(haptic_data, max_len)
                robot_data = pad_with_last_value(robot_data, max_len)
                mydata = np.hstack((haptic_data, robot_data))

                if mydata is not None:
                    output_1, output_2 = results_lookup[output_key]
                    output_vector = np.array([output_1, output_2])
                    all_X_data.append(mydata)
...
def save_final_model(save_dir, model, history, train_config):
    ...
    final_name = f"final_1DCNN_{timestamp}_epoch{n_epochs}_loss{final_dev_loss:.4f}.weights.h5"
...
def main(dense_layers,dropout,filter_size,dilation_rate,kernel_size):
    ...
    DATA_BASE_PATH_ROOT = r"/scratch/users/sehui/CS230/DATA/"
    PROCESSED_DATA_FILE = "processed_hapticrobot_data_cnn_sherlock.npy"
    log_dir = "logs"

    LOG_FILE = f"{log_dir}/experiment_log_hapticrobot_1dcnn.csv"
...
if __name__ == "__main__":
    for dropout in [0,0.1,0.2]:
        for dense_layers in [[64],[32,64],[32,64,128]]:
            for (filter_size,dilation_rate) in [([64,64,64],[1,4,16]),([32,64,128],[1,4,4]),([32,64,64],[1,2,4]),([64,64,64,64,64],[1,2,4,8,16]),([32,64,128,64,32],[1,2,4,2,1]),([32,32,64,64,64],[1,2,4,4,4])]:
